...
(() => {
  ...
  // ============================================================
  // THE MOTION BENCH — skills chapter
  //
  // The six easing tokens in css/styles.css :root, drawn and played from
  // their own control numbers. A CSS transition would animate the square
  // correctly but tell us nothing about WHERE it is mid-flight, and the
  // playhead has to sit on the curve at exactly the square's progress —
  // so the cubic-bezier timing function is solved here instead, once per
  // frame, and drives both.
  //
  // A CSS cubic-bezier(x1,y1,x2,y2) is a curve through (0,0) and (1,1)
  // whose x axis is time and y axis is progress. Given elapsed time we
  // solve x(u) = t for the parameter u (Newton-Raphson, bisection as the
  // safety net), then read y(u) — the eased progress.
  // ============================================================
  const EASINGS = [
    { name: 'spring', p: [0.34, 1.4, 0.5, 1] },
    { name: 'glide',  p: [0.34, 1.2, 0.5, 1] },
    { name: 'lift',   p: [0.25, 0.46, 0.45, 0.94] },
    { name: 'pop',    p: [0.34, 1.56, 0.64, 1] },
    { name: 'rise',   p: [0.22, 1, 0.36, 1] },
    { name: 'draw',   p: [0.4, 0, 0.2, 1] },
  ];
  const BENCH_MS = 900;

  // one axis of a cubic Bézier with fixed endpoints 0 and 1
  const bez = (u, a, b) => {
    const v = 1 - u;
    return 3 * v * v * u * a + 3 * v * u * u * b + u * u * u;
  };
  const bezPrime = (u, a, b) => {
    const v = 1 - u;
    return 3 * v * v * (a - 0) + 6 * v * u * (b - a) + 3 * u * u * (1 - b);
  };

  function easeAt(t, [x1, y1, x2, y2]) {
    if (t <= 0) return 0;
    if (t >= 1) return 1;
    let u = t;                                  // x ≈ u is a good first guess
    for (let i = 0; i < 6; i++) {
      const d = bezPrime(u, x1, x2);
      if (Math.abs(d) < 1e-6) break;            // flat — Newton would blow up
      const err = bez(u, x1, x2) - t;
      if (Math.abs(err) < 1e-6) return bez(u, y1, y2);
      u -= err / d;
    }
    // Newton wandered out of range (it can, on the flat-start curves) —
    // fall back to bisection, which cannot
    let lo = 0, hi = 1;
    u = t;
    for (let i = 0; i < 24; i++) {
      const x = bez(u, x1, x2);
  ...
  if (benchRow) {
    ...
    const BOX = 64;                             // the unit square, in svg units
    ...
    // svg y runs downward, so progress 1 sits at y = 0
    const X = (v) => v * BOX;
    const Y = (v) => BOX - v * BOX;

    const items = EASINGS.map(({ name, p }) => {
      const [x1, y1, x2, y2] = p;
      const btn = document.createElement('button');
      btn.className = 'bench-ease';
      btn.type = 'button';
      btn.setAttribute('aria-label', 'Play the ' + name + ' curve');

      // The viewBox holds the unit square plus the little headroom the
      // overshooting curves actually use (~6 units) — NOT the control
      // points, which sit as far as 1.56 up and would shrink every curve
      // on the row to fit one of them. The four numbers are printed below
      // instead, where they read better than a hairline ever would.
      const svg = el('svg', { class: 'bench-curve', viewBox: '-6 -12 76 82' });
      svg.appendChild(el('rect', { class: 'bench-frame', x: 0, y: 0, width: BOX, height: BOX }));
      svg.appendChild(el('path', {
        class: 'bench-path',
        d: `M0 ${Y(0)} C ${X(x1)} ${Y(y1)}, ${X(x2)} ${Y(y2)}, ${X(1)} ${Y(1)}`,
      }));
      const head = el('circle', { class: 'bench-head', cx: 0, cy: Y(0), r: 3.2 });
      ...
      meta.append(nameEl, val);
      btn.appendChild(meta);

      const track = document.createElement('span');
      track.className = 'bench-track';
      const square = document.createElement('span');
      square.className = 'bench-square';
      track.appendChild(square);
      btn.appendChild(track);

      benchRow.appendChild(btn);
      return { btn, head, square, p, raf: 0 };
    });

    function play(item) {
      if (reduced) return;                      // no motion: the curve is the point
      cancelAnimationFrame(item.raf);
      item.btn.classList.add('is-playing');
      const t0 = performance.now();
      const step = (now) => {
        const t = Math.min((now - t0) / BENCH_MS, 1);
        const eased = easeAt(t, item.p);
        // the playhead walks the curve: x is time, y is that eased progress
        item.head.setAttribute('cx', X(t));
        item.head.setAttribute('cy', Y(eased));
        item.square.style.setProperty('--t', eased);
        if (t < 1) { item.raf = requestAnimationFrame(step); return; }
        item.raf = 0;
        // settle: hold the finished state a beat, then rewind if the cursor
        // has moved on (a curve left mid-flight would read as a bug)
        setTimeout(() => {
          if (item.btn.matches(':hover, :focus-visible')) { play(item); return; }
          item.btn.classList.remove('is-playing');
          item.square.style.setProperty('--t', 0);
          item.head.setAttribute('cx', 0);
          item.head.setAttribute('cy', Y(0));
        }, 420);
      };
    ...
    items.forEach((item) => {
      item.btn.addEventListener('pointerenter', () => play(item));
      item.btn.addEventListener('focus', () => play(item));
      item.btn.addEventListener('click', () => play(item));
    });

    // When the chapter first arrives, deal the whole row once — six squares
    // leaving together and landing apart says what six curves are for far
    // faster than the caption does.
    const skills = document.getElementById('skills');
    if (skills && !reduced) {
      let dealt = false;
      const deal = () => {
        if (dealt || !skills.classList.contains('revealed')) return;
        dealt = true;
        items.forEach((item, i) => setTimeout(() => play(item), 260 + i * 110));
      };
      if (skills.classList.contains('revealed')) deal();
      else new MutationObserver(deal)
        .observe(skills, { attributes: true, attributeFilter: ['class'] });
    }
  }

  // ============================================================
  // THE SPINE — resume chapter. The bars mirror the ledger, so hovering
  // either half lights the other; a row taking keyboard focus lights its
  // bar too, which is why the spine itself stays out of the tab order.
  // ============================================================
  const bars = Array.from(document.querySelectorAll('.spine-bar'));
  const xpRows = Array.from(document.querySelectorAll('.xp-row'));
  if (bars.length && bars.length === xpRows.length) {
    const link = (i, on) => {
      bars[i].classList.toggle('is-lit', on);
      xpRows[i].classList.toggle('is-open', on);
    };
    bars.forEach((bar, i) => {
      bar.addEventListener('pointerenter', () => link(i, true));
      bar.addEventListener('pointerleave', () => link(i, false));
    });
    xpRows.forEach((row, i) => {
      // the row already opens itself on :hover/:focus-visible — this only
      // has to carry the light down to the bar
      row.addEventListener('pointerenter', () => bars[i].classList.add('is-lit'));
      row.addEventListener('pointerleave', () => bars[i].classList.remove('is-lit'));
      row.addEventListener('focus', () => bars[i].classList.add('is-lit'));
      row.addEventListener('blur', () => bars[i].classList.remove('is-lit'));
    });
  }
